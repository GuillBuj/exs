...
public class Fabrique {
    ...
    private static Set<IBloc> creerBloc(final Callable<IBloc> task, final int nb){
    ...
    public static Set<IBloc> creerMur(final int nb){
        Callable<IBloc> taskMur = ()-> {return new Mur(1,1,1,true);};
        return creerBloc(taskMur,nb);
    }

    public static Set<IBloc> creerPorte(final int nb){
        Callable<IBloc> taskPorte = ()-> {return new Porte(1,1,1,false);};
        return creerBloc(taskPorte,nb);
    }

    public static Set<IBloc> creerToit(final int nb){
        Callable<IBloc> taskToit = ()-> {return new Toit(1,1,1);};
        return creerBloc(taskToit,nb);
    }
